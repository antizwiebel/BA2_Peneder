    ...
    */
    ...
    func readFromCSVFile(rules: [Rule]?)  -> [Rule]{
        ...
        if let csv = try? CSVReader(stream: stream!, hasHeaderRow: true) {
            ...
            var readRule = Rule()
            // get fuzzyvalues from file, if this does not work, fall back to example fuzzy values
            let fuzzyValues = FuzzyValue.retrieveFuzzyValuesFromCategoryFile() ?? FuzzyValue.retrieveExampleFuzzyValues()
            if fuzzyValues.isEmpty == false {
                while csv.next() != nil {
                    switch csv["operator"]! {
                    case "C":
    ...
    func getFuzzyValueForTitle(title: String, fuzzyValues: [FuzzyValue]) -> FuzzyValue {
        let itemIndex = fuzzyValues.index(where: { (item: FuzzyValue) -> Bool in
            item.title == title // test if categories have same title
        })
